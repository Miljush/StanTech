...
const MajstorRegister = () => {
  ...
  const handleClose = () => {setShow(false);setRedirect(true)};
  ...
  const handleUslugaChange = (e, index) => {
    const updatedUsluge = [...usluge];
    updatedUsluge[index] = e.target.value;
  ...
  async function registerUser(ev) {
    ...
    if(profilePicture.length==0){
      try {
        await axios.post("/registerMajstor", {
          user,
          ime,
          prezime,
          brtelefona,
  ...
  if(redirect){
    ...
    <form className="max-w-md mx-auto" onSubmit={registerUser}>
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>Registrovali ste se! Sada mozete da se ulogujete.</Modal.Body>
        <Modal.Footer>
      ...
      <Modal show={show2} onHide={handleClose2}>
        ...
        <Modal.Body>Greška prilikom registracije. Pokušajte ponovo kasnije.</Modal.Body>
        <Modal.Footer>
      ...
      {usluge.map((usluga, index) => (
        <div key={index}>
          <input
            ...
            value={usluga}
            onChange={(e) => handleUslugaChange(e, index)}
            required
          />
          <button type="button" className="bg-blue-500 text-white py-2 px-4 rounded-full mb-2" onClick={() => removeUsluga(index)}>
            Ukloni
          </button>
        </div>
      ))}

      <button type="button" className="bg-blue-500 text-white py-2 px-4 rounded-full mb-2" onClick={addUsluga}>
        Dodaj uslugu
      </button>
...
export default MajstorRegister;